...
def customer(env, name, arrival_time, service_time, server):
    # Wait until the customer's arrival time
    ...
    with server.request() as req:
        ...
        # Time the customer waits in the queue
        ...
        time_in_system.append(end - arrival)

        print(
            f"Customer {name} | Arrival: {arrival} | "
            f"Start: {start} | End: {end} | Wait: {start - arrival}"
        )
...
for i in range(n):
    for j in range(n):
        # If customer i finishes before customer j starts
